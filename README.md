...
* **Garantia de Qualidade Multi-Camadas (Shift-Left QA):**
  * **Unidade Python:** Handlers, Schemas Pydantic, S3 Event Processors, Cache, Circuit Breaker e SSM Manager testados com `pytest-mock`, `moto v5` e `event_factory` (32+ testes unitários).
  * **Integração Real:** DAO testado contra container oficial do DynamoDB Local rodando via **Testcontainers**.
  * **Infraestrutura Java:** Testes de asserção da stack CDK (SSM, S3, VPC, ElastiCache, DynamoDB) usando **JUnit 5**.

---

## 🏛️ Arquitetura Atual do Projeto (Módulo 07)

A cada módulo concluído, a arquitetura evolui de forma incremental. O diagrama abaixo representa o estado atual do sistema com borda tratada, gerenciamento de configurações no SSM Parameter Store, disjuntor distribuído, upload de mídias via Presigned URLs, processamento por eventos S3, caching em memória com ElastiCache e resiliência no DynamoDB:

![AWS Serverless Product API Architecture v5](./docs/05-advanced-lambda-patterns/architecture_v5.png)
...
│   ├── shared/                         # ErrorClassifier, CircuitBreaker, SSMManager, Resilience & Utilities
│   ├── tests/                          # Suíte de Testes Automatizados Python
│   │   ├── integration/                # Testes de Integração com Testcontainers + DynamoDB
│   │   └── unit/                       # Testes Unitários dos Handlers, Resiliência, SSM e Circuit Breaker
...
│   ├── main/java/com/petreca/          # Stack Principal (ProductApiStack - DynamoDB, S3, Valkey, VPC, SSM)
...
| :---: | :--- | :--- | :---: | :---: |
| `POST` | `/products` | Cadastra um novo produto no DynamoDB | `201 Created` | `400` / `500` / `503` |
...
```bash
# Executar Testes Unitários Python (Handlers, Schemas, Cache, SSM Manager & Circuit Breaker)
...
Como esta arquitetura utiliza recursos empresariais de nuvem privada (**VPC, Security Groups, ElastiCache, SSM Parameter Store e S3 Lifecycle Rules**), a validação E2E com chamadas `curl` é realizada diretamente na AWS Cloud:
...
# 3. Leitura direta de parâmetros no AWS Systems Manager
